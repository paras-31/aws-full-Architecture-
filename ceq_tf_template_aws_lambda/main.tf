...
#   image_uri                          = var.image_uri
  ...
  source_code_hash = var.ignore_source_code_hash
...
resource "aws_iam_role" "lambda_role" {
  for_each = var.create_s3_notification ? var.s3_bucket_notification : {}
  name = each.value.role_name # Ensure this is valid
  assume_role_policy = data.aws_iam_policy_document.lambda_assume_role.json
}

resource "aws_iam_policy" "lambda_s3_invocation" {
  for_each = var.create_s3_notification ? var.s3_bucket_notification : {}
...
resource "aws_iam_role_policy_attachment" "lambda_s3_invocation_attach" {
  for_each = var.create_s3_notification ? var.s3_bucket_notification : {}

  role       = aws_iam_role.lambda_role[each.key].name
  policy_arn = aws_iam_policy.lambda_s3_invocation[each.key].arn
}
...
resource "aws_s3_bucket_notification" "this" {
  ...
  depends_on = [
    ...
    aws_iam_role_policy_attachment.lambda_s3_invocation_attach
  ]
}
...
resource "null_resource" "sam_metadata_aws_lambda_function" {
  ...
  triggers = {
    ...
    built_output_path = "var.source_file"
  }
